...
def plot_stat(data_source, fig_location=None, show_figure=False):
    ...
    for reg in np.unique(data_source["region"]):
        reg_arr.append(
            [
                np.where(data_source["region"] == reg)[0][0],
                np.where(data_source["region"] == reg)[0][-1],
                reg,
            ]
        )
    ...
    # iterujeme cez regiony a ziskame counts pre unikatene hodnoty
    for i, reg_i in enumerate(reg_arr):
        uniques, counts = np.unique(
            data_source["p24"][reg_i[0]: reg_i[1]], return_counts=True
        )
        for n, uni in enumerate(
            uniques
        ):  # ulozenie danych countov ku regionu a unikatnej hodnote
    ...
    # inicializacia figure a subplotov (2 riadky, 1 stlpec o velkosti 11x7)
    ...
    im1 = ax1.imshow(
        ...
        cmap="viridis",
        norm=clr.LogNorm(vmin=1, vmax=10 ** 5)
        )
    ...
    # normalizacia riadkov - iterovanie len cez vypocitane data
    for line in end:
...
def main():
    ...
    parser.add_argument(
        "--fig_location",
        help="path and filename where to save stats",
        action="store"
    )
    parser.add_argument(
        "--show_figure",
        help="use if you want to show stats",
        action="store_true"
    )
